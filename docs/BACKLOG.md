...
| BL-30 | **Seis das sete facetas de tag estão dormentes.** `format`, `occasion`, `vibe`, `logistics`, `dietary` e `character` têm zero atribuições nos publicados, então o painel da F-04 não as renderiza | Não é bug: por desenho, faceta sem opção populada não aparece, e elas surgem sozinhas conforme o Michael taggear, sem deploy. Registrado porque é o sintoma visível de que a curadoria (Bíblia §13.1) é o caminho crítico — o filtro está pronto e esperando o dado. **Atualizado duas vezes na S08.** Com a faceta de rating removida (RN-30) e as tags `suggested` escondidas do público (RN-31), o painel hoje oferece **estrela, tipo e área — três facetas, nenhuma de tag**. As 5 cuisines que apareciam eram todas palpite do import e saíram junto. **A medição que fecha o assunto:** das 145 atribuições, **zero** são do curador; dos 58 publicados, 11 têm alguma tag e nenhum tem `the_dish`; 21 das 94 tags foram usadas alguma vez. E o Edu **não pode taggear no lugar do Michael** — ele nunca esteve em nenhum desses lugares. Ou seja: isto não é dívida esperando uma sessão de CLI, é trabalho que depende de uma pessoa específica. Ver `BL-34` para o que dá para destravar sem ela |
...
| BL-34 | **O conteúdo travou numa pessoa só, e vale separar o que dá para destravar sem ela.** Constatado na S08: o Edu não pode taggear nem escrever `the_dish` — ele nunca esteve em nenhum dos 511 lugares | **Parte da máquina: feita na S08.** A migration `20260807140000_suggest_cuisine_published` sugeriu 28 cuisines, levando os publicados de comida sem cuisine de 45 para **17**. Tudo `suggested`, invisível ao visitante pela RN-31, esperando aprovação em lote no admin. Rollback escrito e cirúrgico: apaga só estas 28, não toca as 145 do import nem nada que o Michael já tenha confirmado (confirmar vira `curator` e o rollback pula). Os 17 que sobraram estão nominados no rodapé da migration — são os que nem o nome nem conhecimento público resolvem. Só `cuisine` e parte de `format` são inferíveis de nome e tipo; `vibe`, `logistics`, `dietary`, `occasion` e **toda** a `character` exigem ter estado no lugar e **não devem ser geradas** — seriam falsas, e falsas na voz do Michael. **O que só ele pode fazer, e é menor do que parece:** `the_dish` e `curator_note` em 8-10 dos lugares mais fortes. Não precisa abrir nada — são duas perguntas de memória por lugar ("o que eu peço aqui?", "por que este importa?"). Ideia registrada e não executada: preparar essa lista curta num formato que o Edu mande por mensagem e cole a resposta de volta |
...
| OP-04 | **Deploy na Vercel.** A Bíblia §3 previa "a configurar após a F-03" e ficou para trás | 🟡 **Repositório pronto na S08; falta o passo que exige a conta do Edu.** Já commitado: `vercel.json` com o rewrite de SPA — **sem ele todo link profundo (`/city/austin`, `/place/canje`) devolve 404**, que é exatamente o que um link compartilhado é; `public/robots.txt` reforçando o ADR-07 para crawler que não executa JS; e `engines.node >= 22` no `package.json`, senão o Vercel pode escolher um Node velho demais para o Vite 8. Falta: importar o repo no painel da Vercel e colar as duas variáveis `VITE_` (valores em `.env.local`). Passo a passo no log da S08 |
