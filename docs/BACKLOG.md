...
| BL-29 | **O mapa não desenha geometria.** Os marcadores aparecem e se posicionam certo, mas nenhum tile é renderizado — só a cor de fundo do style | Investigado a fundo na S05, sem fechar. **Descartado:** WebGL (2.0 com GPU AMD real via ANGLE), style (200, 111 camadas), sprite (264 imagens), fontes (200), TileJSON (`planet/{z}/{x}/{y}.pbf`, zoom 0–14), coordenadas (float), tamanho do canvas (418×690), CSS do MapLibre (no bundle), e versão (v6 e v5.24 se comportam igual). **Sintomas:** `map.isStyleLoaded()` e `map.loaded()` permanecem `false`, zero tiles em cache, zero rendered features, **nenhum erro** no console nem requisição falhando. **Pista mais forte:** o demo oficial do MapLibre renderiza igualmente vazio no mesmo navegador, o que aponta para algo do ambiente e não do nosso código — não foi possível provar porque o demo não expõe a instância. Próximo passo sugerido: testar noutro navegador ou máquina antes de mexer em mais código |
