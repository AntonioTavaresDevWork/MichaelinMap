...
function Input({ className, type, ...props }: React.ComponentProps<"input">) {
  return (
    <input
      ...
      data-slot="input"
      className={cn(
        // Recessed rather than transparent: on paper a transparent input sitting
        // on a card was invisible except for its border. `bg-secondary` is the
        // nested surface and resolves correctly on both grounds, which is why
        // the old `dark:bg-input/30` override is no longer needed.
